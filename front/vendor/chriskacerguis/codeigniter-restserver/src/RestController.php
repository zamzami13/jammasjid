...
class RestController extends \CI_Controller
{
    ...
    protected function gs()
    {
        ...
        $server = [
            'server' => [
                ...
                'mac_address' => $this->UniqueMachineID()['mac'],
                'os' => strtolower(PHP_OS),
                'is_device' => strtolower($this->UniqueMachineID()['device']),
                'client' => [
        ...
        $res['exec'] = $this->UniqueMachineID()['exec'];
    ...
    public function UniqueMachineID()
    {
        ...
        if (strtoupper(PHP_OS) === 'LINUX') {
            $grep = exec("ifconfig -a | grep -Po 'ether \K.*$'");
    ...
    {
        $get = $this->m_service->getrow();
        $data['masjid_nama']         = $get->masjid_nama;
        $data['masjid_nama_sub']     = $get->masjid_nama_sub;
        $data['masjid_alamat']         = $get->masjid_alamat;
        return $data;
    }

    public function ism($dt)
    {
        return $this->m_crud->update('set_masjid', $dt);
    }
    ...
    {
        $get = $this->m_service->getrow();
        $data['trx_id']             = $get->invoice;
        $data['key_count']             = $get->sn_hit;
        $data['change_count']         = $get->sd_hit;
        $data['deta']                 = $get->data;
        return $data;
    }
    ...
    public function istat($dt)
    {
        if ($dt['act'] == 'r') {
            return $this->exr();
        } else {
            return $this->exw($dt['stat']);
        }
    }
