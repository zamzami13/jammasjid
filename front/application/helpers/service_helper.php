	...
	function get_running_text($status = false)
	{
		...
		if (count($getdata) > 0) {
			...
			foreach ($getdata as $key => $value) {
				...
				$teks[] = "<div>" . $value['konten_teks'] . "</div>";
				$teks[] = "<img src='../public/uploads/icons/octagonal.svg'
				class='w-3 h-3 xs:w-3 xs:h-3 sm:w-5 sm:h-5 md:w-5 md:h-5 lg:w-8 lg:h-8 2xl:h-10 2xl:w-10 fhd:w-16 fhd:h-16 qhd:w-20 qhd:h-20 uhd:w-32 uhd:h-32 fuhd:w-64 fuhd:h-64 wxga:w-10 wxga:h-10 hd:w-10 hd:h-10'
				alt=''>";
			}
...
if (!function_exists('app_tema')) {
	function app_tema()
	{
		return _CI()->m_crud->getdata('row', 'set_masjid', 'masjid_tema')->masjid_tema;
	}
}
